...
function ccgen(bin, mes, anio, cvc, cantidad) {
    init();
    darkside(bin, 1000, cantidad, mes, anio, cvc);
}
...
function darkside(p1, tr, cantidad_post, fech_post_mes, fech_post_year, cvc) {
    tr *= 1.0;
    if (tr < 1 || tr == null) tr = 1;
    var ccghm = cantidad_post;
    if (ccghm < 1) ccghm = 1;
    ...
    if (p1 != "" && p1 != null) {
        ...
        for (var k = 1; k <= ccghm; k++) {
            ...
            if (clcd && ccck) {
                ...
                if (fech_post_mes == "rnd" && fech_post_year == "rnd") {
                    var dnowi = new Date();
                    var mes = unilenS(rnd(1, 12), 2, "0", 0);
                    var year = dnowi.getFullYear() + rnd(2, 6);
                } else if (fech_post_mes != "rnd" && fech_post_year == "rnd") {
                    var dnowi = new Date();
                    var mes = fech_post_mes;
                    var year = dnowi.getFullYear() + rnd(2, 5);
                } else if (fech_post_mes == "rnd" && fech_post_year != "rnd") {
                    var dnowi = new Date();
                    var mes = unilenS(rnd(1, 12), 2, "0", 0);
                    var year = fech_post_year;
                } else if (fech_post_mes != "rnd" && fech_post_year != "rnd") {
                    var mes = fech_post_mes;
                    var year = fech_post_year;
                } else var ccexp = "";
                ...
                if (jou == 0) {
                    var out = (out += cdif);
                    if (checked == 1) var out = (out += "|" + mes + "|" + year);
                    if (checked == 1) var out = (out += "|" + eccv);
                    if (checked == 0) var out = (out += "|" + cn + "\n");
                    else var out = (out += "\n");
                }
...
function sbtStringSpRnd(s1, s2, bS) {
    ...
    for (var i = 1; i <= s1.length; i++) {
        ...
        var c2 = s2.indexOf(c1);
        if (c2 == -1) ous += c1;
        else ous += bS[Math.floor(Math.random() * bS.length)];
    }
    return ous;
}
