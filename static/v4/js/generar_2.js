$(document).ready(function() {
    ...
    function recursivePost(indice) {
        ...
        formData.append('lista', CardElements);
        formData.append('cookies', cookie);
        formData.append('pais', pais);

        $.ajax({
            url: '/gates/amazon_api.php',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            success: function(data) {
                try {
                    ...
                    let deleteLineAux = $('#result').val().split('\n');
                    ...
                    if (data.status === "success") {
                        ...
                        $("#viva").append(`
                            <tr>
                                <td>
                                    <div style="background:#23272f;padding:10px 14px;margin-bottom:10px;border-radius:8px;border:1px solid #333;color:#b6eaff;">
                                        <div style="font-size:16px;font-weight:bold;color:#7fff7f;">${data.card} ✅</div>
                                        <div style="margin:4px 0 2px 0;">
                                            <span style="color:#fff;">Estado:</span> <b style="color:#7fff7f;">${data.message}</b>
                                        </div>
                                        <div style="margin:2px 0;">
                                            <span style="color:#fff;">Removido:</span> <b>${data.removed}</b>
                                        </div>
                                    </div>
                                </td>
                            </tr>
                        ...
                        if (data.message.includes("Cookies") || data.message.includes("Dirección")) {
                            errorNum++;
                            $("#error").append(`<tr><td>${data.card} ⚠️ ${data.message}</td></tr>`);
                            $('#error_num').html(errorNum);
                        } else {
                            deadsNum++;
                            $("#muerta").append(`<tr><td>${data.card} ❌ ${data.message}</td></tr>`);
                    ...
                    const width = ((indice / Cards.length) * 100).toFixed(2);
                    document.getElementById("porcentaje").style.width = width + '%';
                    $('#porcentaje').html(width + '% Completado');

                    // Continuar con la siguiente tarjeta
                    if (running) {
                        setTimeout(() => recursivePost(indice), 1000); // Añadir un delay de 1 segundo entre peticiones
                    }

                } catch (error) {
                    console.error('Error procesando respuesta:', error);
                    errorNum++;
                    $("#error").append(`<tr><td>${CardElements} ⚠️ Error procesando respuesta</td></tr>`);
                    $('#error_num').html(errorNum);
                    
                    if (running) {
                        setTimeout(() => recursivePost(indice), 1000);
                    }
                }
            },
            error: function(xhr, status, error) {
                ...
                errorNum++;
                $("#error").append(`<tr><td>${CardElements} ⚠️ Error en la petición: ${error}</td></tr>`);
                $('#error_num').html(errorNum);
