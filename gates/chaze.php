<?php $site_page = "Chaze Gate"; $path = $_SERVER["DOCUMENT_ROOT"]; include($path."/static/v4/plugins/form/header.php"); ?>
<section class="content">
   ...
   <div class="content__inner">
      <div class="card">
         <div class="card-body">
            <h4 class="card-title"><i class="zwicon-credit-card"></i> Chaze Gate <span class="badge-shadow badge-primary">Live <span class="badge-shadow badge-pill badge-light">4</span></span> <span class="badge-shadow badge-danger">Die <span class="badge-shadow badge-pill badge-light">0</span></span></h4>
            <div class="row">
               ...
               <div class="col-md-12">
                  <div class="col-md-12">
                     <div class="progress progress-striped text-center" id="barra_porcentaje" style="background-color: #0a2624; display: block;">
                        <div class="progress-bar progress-bar-success" id="porcentaje" style="background-color: black; width: 100%; display: block;">Welcome to Chaze Gate!</div>
                     </div>
                  </div>
               </div>
               <div class="col-md-12">
                     ...
                     <div class="card-body">
                        <div class="panel-heading">
                           <h4 class="panel-title">
                              <b>Lives</b> 
                              <div id="chaze-lives-count" class="val-lives btn btn-sm btn-icon btn-circle btn-inverse">0</div>
                           </h4>
                        </div>
                        <div id="chaze-lives" class="table-responsive" style="max-height: 120px; overflow-y: auto;"></div>
                     </div>
                  </div>
                  <div class="card">
                     <div class="card-body">
                        <div class="panel-heading">
                           <h4 class="panel-title">
                              <b>Dies</b> 
                              <div id="chaze-dies-count" class="val-dies btn btn-sm btn-icon btn-circle btn-inverse">0</div>
                           </h4>
                        </div>
                        <div id="chaze-dies" class="table-responsive" style="max-height: 120px; overflow-y: auto;"></div>
                     </div>
                  </div>
                  <div class="card">
                     <div class="card-body">
                        <div class="panel-heading">
                           <h4 class="panel-title">
                              <b>Errores</b> 
                              <div id="chaze-errors-count" class="val-errors btn btn-sm btn-icon btn-circle btn-inverse">0</div>
                           </h4>
                        </div>
                        <div id="chaze-errors" class="table-responsive" style="max-height: 120px; overflow-y: auto;"></div>
                     </div>
                  </div>
               </div>
            </div>
...
function addToChazeList(type, card, result) {
    let html = `<span style='color:#00fff7;font-weight:bold;'>${card}</span> <span style='color:#fff;'>➔ ${result}</span>`;
    if(type === 'live') {
        chazeLivesNum++;
        document.getElementById('chaze-lives').innerHTML += `<div style='margin-bottom:2px;'>${html}</div>`;
        document.getElementById('chaze-lives-count').textContent = chazeLivesNum;
    } else if(type === 'die') {
        chazeDiesNum++;
        document.getElementById('chaze-dies').innerHTML += `<div style='margin-bottom:2px;'>${html}</div>`;
        document.getElementById('chaze-dies-count').textContent = chazeDiesNum;
    } else {
        chazeErrorsNum++;
        document.getElementById('chaze-errors').innerHTML += `<div style='margin-bottom:2px;'>${html}</div>`;
        document.getElementById('chaze-errors-count').textContent = chazeErrorsNum;
    }
}
...
async function checkCards(cards) {
    for (let i = 0; i < cards.length && isChecking; i++) {
        ...
        try {
            const response = await fetch(`/gates/chaze_check.php?card=${encodeURIComponent(card)}`);
            const data = await response.json();
            
            checkedCards++;
            if (data.status1 === "Verified") {
                liveCards++;
                addToChazeList('live', card, data.status2);
            } else {
                dieCards++;
                addToChazeList('die', card, data.status2);
            }
            ...
            addToChazeList('error', card, 'Error en la verificación');
            checkedCards++;
            updateStats();
        }
    }
