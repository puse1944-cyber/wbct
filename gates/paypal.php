<?php $site_page = "PayPal Gate"; $path = $_SERVER["DOCUMENT_ROOT"]; include($path."/static/v4/plugins/form/header.php"); ?>
<section class="content">
   ...
   <div class="content__inner">
      <div class="card">
         <div class="card-body">
            <h4 class="card-title"><i class="zwicon-credit-card"></i> PayPal Gate <span class="badge-shadow badge-primary">Live <span class="badge-shadow badge-pill badge-light">4</span></span> <span class="badge-shadow badge-danger">Die <span class="badge-shadow badge-pill badge-light">0</span></span></h4>
            <div class="row">
            ...
            <div class="d-flex flex-wrap justify-content-center mb-2" style="gap: 6px;">
               <button type="button" id="iniciar" name="iniciar" class="btn btn-theme btn-sm btn--icon-text" style="min-width: 100px;"><i class="fas fa-play"></i> Iniciar</button>
               <button type="button" id="detener" name="detener" class="btn btn-theme btn-sm btn--icon-text" style="min-width: 80px;"><i class="fas fa-stop"></i> Detener</button>
               </div>
               ...
               <div class="col-md-12">
                  <div class="col-md-12">
                     <div class="progress progress-striped text-center" id="barra_porcentaje" style="background-color: #0a2624; display: block;">
                        <div class="progress-bar progress-bar-success" id="porcentaje" style="background-color: black; width: 100%; display: block;">Welcome to PayPal Gate!</div>
                     </div>
                  </div>
               </div>
               <div class="col-md-12">
                     ...
                     <div class="card-body">
                        <div class="panel-heading">
                           <h4 class="panel-title">
                              <b>Lives</b> 
                              <div id="paypal-lives-count" class="val-lives btn btn-sm btn-icon btn-circle btn-inverse">0</div>
                           </h4>
                        </div>
                        <div id="paypal-lives" class="table-responsive" style="max-height: 120px; overflow-y: auto;"></div>
                     </div>
                  </div>
                  <div class="card">
                     <div class="card-body">
                        <div class="panel-heading">
                           <h4 class="panel-title">
                              <b>Dies</b> 
                              <div id="paypal-dies-count" class="val-dies btn btn-sm btn-icon btn-circle btn-inverse">0</div>
                           </h4>
                        </div>
                        <div id="paypal-dies" class="table-responsive" style="max-height: 120px; overflow-y: auto;"></div>
                     </div>
                  </div>
                  <div class="card">
                     <div class="card-body">
                        <div class="panel-heading">
                           <h4 class="panel-title">
                              <b>Errores</b> 
                              <div id="paypal-errors-count" class="val-errors btn btn-sm btn-icon btn-circle btn-inverse">0</div>
                           </h4>
                        </div>
                        <div id="paypal-errors" class="table-responsive" style="max-height: 120px; overflow-y: auto;"></div>
                     </div>
                  </div>
               </div>
            </div>
...
let liveCards = 0;
let dieCards = 0;
let paypalLivesNum = 0;
let paypalDiesNum = 0;
let paypalErrorsNum = 0;
...
function addToPaypalList(type, card, result) {
    let html = `<span style='color:#00fff7;font-weight:bold;'>${card}</span> <span style='color:#fff;'>➔ ${result}</span>`;
    if(type === 'live') {
        paypalLivesNum++;
        document.getElementById('paypal-lives').innerHTML += `<div style='margin-bottom:2px;'>${html}</div>`;
        document.getElementById('paypal-lives-count').textContent = paypalLivesNum;
    } else if(type === 'die') {
        paypalDiesNum++;
        document.getElementById('paypal-dies').innerHTML += `<div style='margin-bottom:2px;'>${html}</div>`;
        document.getElementById('paypal-dies-count').textContent = paypalDiesNum;
    } else {
        paypalErrorsNum++;
        document.getElementById('paypal-errors').innerHTML += `<div style='margin-bottom:2px;'>${html}</div>`;
        document.getElementById('paypal-errors-count').textContent = paypalErrorsNum;
    }
}

const originalIniciar = document.getElementById('iniciar').onclick;
document.getElementById('iniciar').onclick = async function() {
    ...
    document.getElementById('state').textContent = 'Checking...';
    // Limpiar listas
    document.getElementById('paypal-lives').innerHTML = '';
    document.getElementById('paypal-dies').innerHTML = '';
    document.getElementById('paypal-errors').innerHTML = '';
    paypalLivesNum = 0;
    paypalDiesNum = 0;
    paypalErrorsNum = 0;
    document.getElementById('paypal-lives-count').textContent = 0;
    document.getElementById('paypal-dies-count').textContent = 0;
    document.getElementById('paypal-errors-count').textContent = 0;
    for (let card of cards) {
        ...
        try {
            ...
            checkedCards++;
            if (result.result === 'charged' || result.result === 'approved') {
                liveCards++;
                document.getElementById('detectaLive').play();
                addToPaypalList('live', card, result.result.toUpperCase());
            } else if(result.result === 'declined') {
                dieCards++;
                addToPaypalList('die', card, result.result.toUpperCase());
            } else {
                addToPaypalList('error', card, result.error || 'ERROR');
            }
            updateStats();
            document.getElementById('porcentaje').textContent = `${card} - ${result.result.toUpperCase()}`;
            if (result.result === 'charged' && document.getElementById('customCheckDisabled1').checked) {
                break;
            }
            await new Promise(resolve => setTimeout(resolve, 2000));
        } catch (error) {
            paypalErrorsNum++;
            addToPaypalList('error', card, 'ERROR');
            document.getElementById('state').textContent = 'Error occurred!';
        }
    }
    isChecking = false;
    ...
    document.getElementById('state').textContent = 'Check completed!';
};
...
document.getElementById('detener').addEventListener('click', function() {
    ...
    document.getElementById('detener').disabled = true;
    document.getElementById('loader').style.display = 'none';
    document.getElementById('state').textContent = 'Check stopped!';
});
</script>
